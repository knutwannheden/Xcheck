...
public class CheckCfgJavaValidator extends AbstractCheckCfgJavaValidator {
  ...
  /**
   * Checks that a {@link com.avaloq.tools.dslsdk.check.check.Check Check} which has been disabled in the
  ...
  /**
   * Checks if a {@link com.avaloq.tools.dslsdk.check.check.Check Check} is configured or if it is just referenced.
   * 
   * @param configuredCheck
   *          the configured check
  ...
  private boolean isConfigured(final ConfiguredCheck configuredCheck) {
    return configuredCheck.getSeverity() != SeverityKind.INHERIT || !configuredCheck.getParameterConfigurations().isEmpty();
  }

  /**
   * Checks that final catalogs are not configured.
   * 
   * @see {@link com.avaloq.tools.dslsdk.check.check.CheckCatalog#isFinal()}
   * @param catalog
  ...
  /**
   * Checks that a {@link com.avaloq.tools.dslsdk.check.check.Check Check} which has been marked as {@link com.avaloq.tools.dslsdk.check.check.Check#isFinal()
  ...
  public void checkConfiguredSeverityAllowed(final ConfiguredCheck configuredCheck) {
    ...
    if (isConfigured(configuredCheck) && configuredCheck.getCheck() != null 
      ...
      final com.avaloq.tools.dslsdk.check.check.SeverityKind configuredSeverity = 
        com.avaloq.tools.dslsdk.check.check.SeverityKind.getByName(configuredCheck.getSeverity().getName());
      
      final com.avaloq.tools.dslsdk.check.check.SeverityKind minSeverity = 
          com.avaloq.tools.dslsdk.check.check.SeverityKind.get(
              Math.min(range.getMinSeverity().getValue(), range.getMaxSeverity().getValue()));
      final com.avaloq.tools.dslsdk.check.check.SeverityKind maxSeverity = 
          com.avaloq.tools.dslsdk.check.check.SeverityKind.get(
      ...
      if (configuredSeverity.compareTo(minSeverity) < 0 || configuredSeverity.compareTo(maxSeverity) > 0) {
        com.avaloq.tools.dslsdk.check.check.SeverityKind temp = minSeverity;
  ...
  public void checkConfigurationEqualsDefault(final ConfiguredCheck configuredCheck) {
    final com.avaloq.tools.dslsdk.check.check.Check check = configuredCheck.getCheck();
    ...
    for (final ConfiguredParameter configParam : configuredCheck.getParameterConfigurations()) {
      try {
        ...
        if (parameterValuesEqual(configParam.getNewValue(), param.getDefaultValue())) {
          info(NLS.bind(Messages.CheckCfgJavaValidator_CONFIGURED_PARAM_EQUALS_DEFAULT, param.getParameter().getName()), configParam, CheckcfgPackage.Literals.CONFIGURED_PARAMETER__NEW_VALUE, ValidationMessageAcceptor.INSIGNIFICANT_INDEX, IssueCodes.CONFIGURED_PARAM_EQUALS_DEFAULT);
        }
  ...
  public void checkConfiguredCatalogUnique(final CheckConfiguration configuration) {
    if (configuration.getCatalogConfigurations().size() < 2) {
      return;
    }
    ...
    for (final ConfiguredCatalog c : getDuplicates(predicate, function, configuration.getCatalogConfigurations())) {
  ...
  public void checkConfiguredParameterUnique(final ConfiguredCheck configuredCheck) {
    ...
    Function<ConfiguredParameter, String> function = new Function<ConfiguredParameter, String>() {
      public String apply(final ConfiguredParameter from) {
        return from.getParameter().getParameter().getName();
      }
    };
