...
public class CheckJavaValidator extends AbstractCheckJavaValidator {
  ...
  ImmutableSet.of(CheckPackage.Literals.XGUARD_EXPRESSION__GUARD, //
      ...
      CheckPackage.Literals.FORMAL_PARAMETER__DEFAULT_VALUE);
  ...
  public Set<EReference> getTypeConformanceCheckedReferences() {
    return Sets.union(super.getTypeConformanceCheckedReferences(), checkTypeConformanceCheckedReferences);
  }
  ...
  public void checkCheckNamesAreUnique(final CheckCatalog catalog) {
    Function<com.avaloq.tools.dslsdk.check.check.Check, QualifiedName> function = new Function<com.avaloq.tools.dslsdk.check.check.Check, QualifiedName>() {
      public QualifiedName apply(final com.avaloq.tools.dslsdk.check.check.Check from) {
    ...
    UniquenessJavaValidationHelper<com.avaloq.tools.dslsdk.check.check.Check> helper = //
    new UniquenessJavaValidationHelper<com.avaloq.tools.dslsdk.check.check.Check>(function, getMessageAcceptor()) {
      @Override
      public String getMessage(final com.avaloq.tools.dslsdk.check.check.Check duplicate) {
        return NLS.bind("Duplicate Check name: {0}", duplicate.getName());
      }
    };

    final Iterable<com.avaloq.tools.dslsdk.check.check.Check> allChecksWithName = Iterables.filter(catalog.getAllChecks(), new Predicate<com.avaloq.tools.dslsdk.check.check.Check>() {
      public boolean apply(final com.avaloq.tools.dslsdk.check.check.Check input) {
  ...
  public void checkCategoryNamesAreUnique(final CheckCatalog catalog) {
    ...
    UniquenessJavaValidationHelper<Category> helper = new UniquenessJavaValidationHelper<Category>(function, getMessageAcceptor()) {
      ...
      public String getMessage(final Category duplicate) {
        return NLS.bind("Duplicate Category name: {0}", duplicate.getName());
      }
    };
  ...
  /** {@inheritDoc} */
  @Check
  public void checkFormalParameterType(final FormalParameter param) {
    if (param.getParameter() != null && param.getParameter().getParameterType() == null) {
      error(Messages.CheckJavaValidator_FORMAL_PARAMETER_TYPE, CheckPackage.Literals.FORMAL_PARAMETER__PARAMETER, IssueCodes.FORMAL_PARAMETER_TYPE);
    }
  }
   ...
   */
  @Check
  public void checkSeverityRangesNotAllowedForFinalCheck(final com.avaloq.tools.dslsdk.check.check.Check check) {
   ...
   */
  @Check
  public void checkDocumentationOnCheck(final com.avaloq.tools.dslsdk.check.check.Check check) {
  ...
  public void checkDocumentationOnImplementation(final Implementation implementation) {
    if (implementation.getName() != null) {
      ...
      if (Strings.isEmpty(documentation)) {
        warning(NLS.bind(Messages.CheckJavaValidator_DOCUMENTATION_MISSING, "Definition", implementation.getName()), CheckPackage.Literals.IMPLEMENTATION__NAME, IssueCodes.MISSING_DOCUMENTATION_ON_IMPLEMENTATION);
      }
    }
  }
   ...
   */
  @Check
  public void checkCheckName(final com.avaloq.tools.dslsdk.check.check.Check check) {
   ...
   */
  private Set<String> includedGrammarNames(final Grammar grammar) {
    final Set<String> result = Sets.newHashSet();
    if (!grammar.getUsedGrammars().isEmpty()) {
      result.addAll(Lists.newArrayList(Iterables.transform(grammar.getUsedGrammars(), new Function<Grammar, String>() {
        public String apply(final Grammar from) {
          return from.getName();
        }
      })));
      for (final Grammar g : grammar.getUsedGrammars()) {
        result.addAll(Lists.newArrayList(includedGrammarNames(g)));
      }
    }
    return result;
  }
  ...
  public void checkLanguageOfIncludedCatalog(final CheckCatalog catalog) {
    ...
    Set<String> includedGrammarNames = includedGrammarNames(grammar);

    final String includedGrammarName = catalog.getIncludedCatalogs().getGrammar().getName();
    if (catalog.getIncludedCatalogs().getGrammar() != grammar && !(includedGrammarNames.contains(includedGrammarName))) {
      error(NLS.bind(Messages.CheckJavaValidator_INVALID_GRAMMAR_OF_INCLUDED_CATALOG, includedGrammarName, grammar.getName()), catalog, CheckPackage.Literals.CHECK_CATALOG__INCLUDED_CATALOGS, IssueCodes.INCLUDED_CATALOG_GRAMMAR_MISMATCH);
    }
  }
  ...
  public void checkImplicitIssuedCheck(final XIssueExpression expression) {
    com.avaloq.tools.dslsdk.check.check.Check container = EcoreUtil2.getContainerOfType(expression, com.avaloq.tools.dslsdk.check.check.Check.class);
    if (container == null) {
      return;
    }
  ...
  public void checkIssueExpressionExists(final Context context) {
    if (context.getContextType() == null || context.getConstraint() == null) {
      return;
    }
   ...
   */
  @Check
  public void checkContextTypeIsEClass(final Context context) {
    final JvmTypeReference contextType = context.getContextType();
    if (contextType == null) {
    ...
    try {
      if (type != null && !EObject.class.isAssignableFrom(Class.forName(type.getIdentifier()))) {
        error(Messages.CheckJavaValidator_CONTEXT_TYPE_IS_ECLASS, context, CheckPackage.Literals.CONTEXT__CONTEXT_TYPE, IssueCodes.CONTEXT_TYPE_IS_ECLASS);
      }
      // CHECKSTYLE:OFF
  ...
  @Check
  public void checkContextTypeIsUnique(final com.avaloq.tools.dslsdk.check.check.Check check) {
    ...
    for (final Context c : check.getContexts()) {
      final JvmTypeReference contextType = c.getContextType();
    ...
    for (String type : mm.keys()) {
      ...
      if (duplicatesForType.size() > 1) {
        for (final Context duplicateContext : duplicatesForType) {
          error(Messages.CheckJavaValidator_CONTEXT_TYPES_UNIQUE, duplicateContext, //
              CheckPackage.Literals.CONTEXT__CONTEXT_TYPE, IssueCodes.CONTEXT_TYPES_NOT_UNIQUE);
        }
      }
    }
  }
   ...
   */
  @Check
  public void checkFormalParameterNamesUnique(final com.avaloq.tools.dslsdk.check.check.Check check) {
    if (check.getFormalParameters().size() < 2) {
      return;
    }
    Predicate<FormalParameter> predicate = new Predicate<FormalParameter>() {
      public boolean apply(final FormalParameter configuredParameter) {
    ...
    for (final FormalParameter p : getDuplicates(predicate, function, check.getFormalParameters())) {
      error(Messages.CheckJavaValidator_DUPLICATE_PARAMETER_DEFINITION, p, CheckPackage.Literals.FORMAL_PARAMETER__PARAMETER, ValidationMessageAcceptor.INSIGNIFICANT_INDEX, IssueCodes.DUPLICATE_PARAMETER_DEFINITION);
    }
   ...
   */
  @Check
  public void checkSeverityRangeOrder(final com.avaloq.tools.dslsdk.check.check.Check check) {
   ...
   * <p>
   * Note that this check works even if {@link #checkSeverityRangeOrder(com.avaloq.tools.dslsdk.check.check.Check)} is violated.
   * </p>
   * 
   * @param check
   *          the check
   */
  @Check
  public void checkDefaultSeverityInRange(final com.avaloq.tools.dslsdk.check.check.Check check) {
    if (check.getSeverityRange() == null) {
    ...
    if (check.getDefaultSeverity().compareTo(minSeverity) < 0 || check.getDefaultSeverity().compareTo(maxSeverity) > 0) {
      ...
      while (temp != null && temp.compareTo(maxSeverity) <= 0) {
        ...
        temp = com.avaloq.tools.dslsdk.check.check.SeverityKind.get(temp.getValue() + 1);
      }
  ...
  public void checkCheckInFinalCatalog(final CheckCatalog catalog) {
    ...
    for (final com.avaloq.tools.dslsdk.check.check.Check check : catalog.getAllChecks()) {
      if (check.isFinal()) {
  ...
  // / These Checks shall disappear once the corresponding features are implemented

  /** {@inheritDoc} */
  @Check
  public void checkIncludedCatalogNotSupported(final CheckCatalog catalog) {
    if (catalog.getIncludedCatalogs() != null) {
      error(NLS.bind(Messages.CheckJavaValidator_UNSUPPORTED_FEATURE, "with"), catalog, CheckPackage.Literals.CHECK_CATALOG__INCLUDED_CATALOGS, IssueCodes.UNSUPPORTED_INCLUDED_CATALOG);
    }
  }
